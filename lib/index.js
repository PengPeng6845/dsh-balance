...
const DEFAULT_WARN_THRESHOLD = 5;
...
function resolveConfig(raw) {
  ...
  const warnThreshold =
    typeof config.warnThreshold === "number" && config.warnThreshold >= 0
      ? config.warnThreshold
      : DEFAULT_WARN_THRESHOLD;
  const alertThreshold =
    typeof config.alertThreshold === "number" && config.alertThreshold >= 0
      ? config.alertThreshold
      : DEFAULT_ALERT_THRESHOLD;
  return { currency, prices, persist, warnThreshold, alertThreshold };
}
...
function aggregateEntryOf(buckets, prices) {
  ...
  return {
    ...
    cost: computeCost(buckets, rate).totalCost,
  };
}
...
export function apply(ctx, rawConfig) {
    ...
    projectionCtx.effect(() =>
      projectionCtx.sessionProjections.onChanged((session, key, value) => {
        ...
        store.record(
          ...
          aggregateEntryOf(buckets, config.prices),
          new Date(),
        );
        schedulePersist();
      }),
  ...
  // Widget data endpoint: GET /usage-cost/summary -> { today, month,
  // persisted, currency, thresholds }. No session identity required.
    ...
    webCtx.effect(() =>
      webCtx.webServer.register({
        ...
        handler(req, res) {
          ...
          const { today, month } = store.summary(new Date());
          const body = JSON.stringify({
            ...
            today: {
              uncachedInputTokens: today.uncachedInputTokens,
              outputTokens: today.outputTokens,
              cacheReadTokens: today.cacheReadTokens,
              cacheWriteTokens: today.cacheWriteTokens,
              totalTokens: totalTokens(today),
              cost: roundMoney(today.cost),
            },
            month: {
              uncachedInputTokens: month.uncachedInputTokens,
              outputTokens: month.outputTokens,
              cacheReadTokens: month.cacheReadTokens,
              cacheWriteTokens: month.cacheWriteTokens,
              totalTokens: totalTokens(month),
  ...
  ctx.tools.register({
    ...
    async execute(_args, exec) {
      ...
      if (!projectionsAvailable) {
        ...
        store.record(String(agent.id ?? "unknown"), entry, new Date());
        store.persist();
      }
      const { today, month } = store.summary(new Date());
      return {
