...
const DEFAULT_BALANCE_REFRESH_MS = 60000;
const ACTIVITY_REFRESH_DEBOUNCE_MS = 20000;
 ...
 * - balanceRefreshMs: baseline poll/cache interval (ms, default 60s).
 *   The balance also refreshes on token activity (debounced ~20s after
 *   the last usage change), so it tracks real spend while the model works.
...
export function apply(ctx, rawConfig) {
  ...
  // SSE fan-out: connected browser widgets receive the snapshot the
  // moment a check changes it (no polling lag).
  const sseClients = new Set();
  let lastBalance = null;
  let lastSentPayload = null;
  const buildSummary = (now) => ({
    status: lastBalance ? "ok" : "unavailable",
    balance: lastBalance
      ? {
          currency: lastBalance.currency,
          totalBalance: lastBalance.totalBalance,
          checkedAt: lastBalance.checkedAt,
          stale: lastBalance.stale === true,
        }
      : null,
    realTodayCost: lastBalance ? store.realTodaySpend(now) : null,
    lowBalance: lastBalance
      ? lastBalance.totalBalance < config.lowBalanceAlert
      : null,
    lowBalanceThreshold: config.lowBalanceAlert,
  });
  const broadcast = () => {
    if (sseClients.size === 0) return;
    const payload = JSON.stringify(buildSummary(new Date()));
  ...
  if (balance) {
    ...
    // Activity-triggered refresh: the balance only moves when the model
    // is billed, so each quiet spell after token activity re-checks the
    // account (debounced; streaming chunks just reset the timer).
    const refreshAfterActivity = ctx.debounce(
      () => void pollBalance(),
      ACTIVITY_REFRESH_DEBOUNCE_MS,
    );
    ctx.inject(["sessionProjections"], (projectionCtx) => {
      projectionCtx.effect(() =>
        projectionCtx.sessionProjections.onChanged((_session, key) => {
          if (key === "tokenUsage") refreshAfterActivity();
        }),
      );
    });
  }

  // Widget data endpoints. The summary route answers the 15s fallback
  // polling; the events route is a held-open SSE stream (the WebRoute
  // contract explicitly supports this) pushing snapshots on every change.
  ctx.inject(["webServer"], (webCtx) => {
    webCtx.effect(() => {
      const offSummary = webCtx.webServer.register({
        kind: "exact",
        ...
        async handler(req, res) {
          ...
          const body = JSON.stringify(buildSummary(now));
          res.writeHead(200, { "content-type": "application/json" });
          res.end(req.method === "HEAD" ? undefined : body);
        },
      });
      const offEvents = webCtx.webServer.register({
        kind: "exact",
        ...
        handler(req, res) {
          ...
          res.write("data: " + JSON.stringify(buildSummary(new Date())) + "\n\n");
          req.on("close", () => {
            sseClients.delete(client);
          });
        },
      });
      const offHeartbeat = webCtx.interval(() => {
        if (sseClients.size === 0) return;
        for (const client of [...sseClients]) {
          try {
